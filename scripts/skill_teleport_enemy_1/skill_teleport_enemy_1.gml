function skill_teleport_enemy_1(unit){
	...
	if(unit.skill_progress==2){
		...
		skill_range = obj_gridCreator.highlighted_support_circle(original_grid._x_coord, original_grid._y_coord, 2);
	}
	...
	if (keyboard_check_pressed(vk_enter) || keyboard_check_pressed(ord("L"))) {
		if(unit.skill_progress==1){
			...
			if(obj_gridCreator.battle_grid[skill_coords[0]][skill_coords[1]]._is_empty){
					...
					original_grid = skill_range_aux[0];
					target_grid = skill_range_aux[1];
					target_unit = original_grid._entity_on_tile;
					target_unit.teleport(target_grid._x_coord,target_grid._y_coord);
